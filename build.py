...
<section class="section"><div class="section-head"><h2>Selected publications</h2>{link('publications.html','All publications')}</div>{''.join(pub(D['publications'][i]) for i in [0,3,6])}</section>
...
<article class="past-card" id="droplets"><img src="assets/droplet.jpg" alt="Sequence showing an oil droplet impacting a layered liquid" loading="lazy"><h3>Droplet impact on layered liquids</h3><p>I contributed to experiments and modeling of oil droplets impacting an oil layer on water, investigating how two deforming interfaces influence crater formation and jet dynamics.</p><div class="links">{link('publications.html#droplet','Journal of Fluid Mechanics · 2021 →')}</div></article></div></section>'''

publications='<div class="page-title"><h1>Publications</h1><p>Research on transport, soft materials, and fluid interfaces.</p><div class="links" style="margin-top:20px">'+link(D['scholar'],'Google Scholar →')+'</div></div><h2 class="subheading">Preprints</h2>'+''.join(pub(p) for p in D['publications'] if p['badge']=='Preprint')+'<h2 class="subheading">Journal articles</h2>'+''.join(pub(p) for p in D['publications'] if p['badge']!='Preprint')+'<p class="small-note" style="margin:24px 0">* Equal contribution.</p>'
